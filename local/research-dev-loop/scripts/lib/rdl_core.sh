...
descriptor_round_file_name_known() {
  local name="$1"
  local known_name
  while IFS= read -r known_name; do
    [[ "${name}" == "${known_name}" ]] && return 0
  done < <(descriptor_round_file_names)
  return 1
}

descriptor_round_path_known() {
  local path="$1"
  local prefix round file extra
  IFS='/' read -r prefix round file extra <<< "${path}"
  [[ "${prefix}" == "rounds" ]] || return 1
  [[ "${round}" =~ ^[0-9][0-9][0-9]$ ]] || return 1
  [[ -n "${file}" && -z "${extra}" ]] || return 1
  descriptor_round_file_name_known "${file}"
}
...
descriptor_policy_for_path() {
  case "$1" in
    ...
    decision-ledger.md)
      ...
      ;;
    rounds/*)
      if descriptor_round_path_known "$1" && [[ "${1##*/}" == "prompt.md" ]]; then
...
descriptor_path_known() {
  ...
  descriptor_session_path_known "${path}" || descriptor_round_path_known "${path}"
}
...
session_protocol_files() {
  ...
  if [[ -d "${session_dir}/rounds" ]]; then
    find "${session_dir}/rounds" -mindepth 2 -maxdepth 2 -type f \
      | sed "s#^${session_dir}/##" \
      | while IFS= read -r path; do
